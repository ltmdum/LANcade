...
export interface WordRushGameOptions {
  onStateChange?: () => void;
...
export interface WordRushGame {
  id: string;
  name: string;
  categories: string[];
  getPhase(): string;
  getState(): Omit<WordRushState, 'game' | 'games'>;
...
/**
 * Create a WordRush game instance.
 * @param options Optional configuration overrides.
 * @returns WordRush game instance.
 */
export function createGame(options: WordRushGameOptions = {}): WordRushGame {
   ...
   */
  function getState(): Omit<WordRushState, 'game' | 'games'> {
    const votes = Array.from(match.votesByPlayer.values());
    const rejectCount = votes.filter((vote) => vote === 'reject').length;
